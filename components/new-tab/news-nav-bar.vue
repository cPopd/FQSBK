<template>
	<view>
		<uniNavBar :statusBar="true" :border="false" :fixed="true" @clickRight="openAdd">
			<!-- 左边 -->
			<block slot="left">
				<view class="nav-left">
			...
			<view class="nav-tab-bar u-f-ajc">
				<block v-for="(tab,index) in tabBars" :key="index">
					<view @tap="change(index)" class="u-f-ajc" :class="{'activ':tabIndex == index}">
					{{tab.name}}
			...
			<block slot="right">
				<view class="nav-right u-f-ajc">
					<view class="icon icon iconfont icon-bianji1">
					</view>
				</view>
			</block>
		</uniNavBar>
